...
TextRendering::TextRendering(const std::string& path, bool isIcon)
{
    if (FT_New_Face(m_ft, path.c_str(), 0, &m_face))
    {
        std::cout << "ERROR::FREETYPE: Failed to load font" << std::endl;
    ...
    glActiveTexture(GL_TEXTURE0);
    glBindTexture(GL_TEXTURE_2D_ARRAY, m_textureArray);
    glTexImage3D(GL_TEXTURE_2D_ARRAY, 0, GL_R8, QUAD_SIZE, QUAD_SIZE, 0x300, 0, GL_RED, GL_UNSIGNED_BYTE, 0);

    FT_GlyphSlot slot = m_face->glyph;

    for (wchar_t c = 0xf000; c < 0xf300; c++)
    {
        // load character glyph 
        if (FT_Load_Char(m_face, c, FT_LOAD_RENDER))
        {
        ...
        m_icons.insert(std::pair<wchar_t, Character>(c, character));
    }
